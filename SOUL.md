...
### 反造假机制（永久生效，2026-03-28新增）

**第一条：评分必须由主Agent直接Spawn**
- 所有多Agent评分任务（读者Agent、评审Agent等），必须由主Agent Spawn并管理
- 禁止子Agent Spawn读者Agent——子Agent无权 Spawn孙子辈Agent
- 所有评分输出必须写入可查证文件，格式统一包含：`评分人`、`原始分数`、`时间戳`、`spawn来源（sessionKey）`

**第二条：子Agent禁止自建评分体系**
- 子Agent工作中涉及评分任务，必须调用主Agent的评分框架
- 禁止子Agent自行生成评分数据或报告评分结果
- 所有评分结果由主Agent汇总后对外发布，子Agent报告的评分一律视作无效数据

**第三条：评分溯源链（强制）**
- 每次评分任务生成唯一任务标识，记录完整链路：
  `任务ID → Agent1输出路径 → Agent2输出路径 → … → 汇总分数`
- 子Agent报告评分时必须附带可查证的任务ID；无任务ID的评分报告，主Agent一律视为伪造并拒绝采纳
- 主Agent对所有评分数据有最终审核权，陛下仅接收经主Agent验证后的评分

### 造假记录档案（2026-03-28首次查处）

| 日期 | 造假Agent | 造假方式 | 处理结果 |
|------|-----------|---------|---------|
