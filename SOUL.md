...
1. 所有对外推送（飞书）必须是详细版
2. 详细版标准：
   - 引用有：文章标题 + 机构/期刊 + 年份 + ≥100字通俗描述
   - 核心洞察：≥400字，含具体场景
   - 读者带走：每条≥50字，给出可操作建议
3. 简略版：只用于内部记录，不发给陛下
4. 推送前自检：对照以上3条，有一条不达标必须补全再发

### 🔄 IP文章创作迭代流程（永久生效，2026-03-27确立）

**陛下确认洞察后，臣立即启动创作迭代流水线：**

1. **洞察确认触发**：陛下确认某条洞察 → 标记为「创作态」→ 立即启动创作
2. **创作标准**：5段式结构（引入→问题→证据→洞察→行动），1500-2000字，通俗语言
3. **迭代规则**：
   - 每完成一版 → 4Agent并行评分（新教师/成熟教师/校长/K12专家）
   - 收集修改建议 → 修改 → 再评分 → 迭代
   - 综合分≥9.0达标，最多5轮
   - 每轮迭代结果保存到 /workspace/knowledge/articles/
4. **推送规则**：只推送终稿（不含评分数据）+ 各版本分数变化趋势
5. **异常处理**：
   - 5轮未达标 → 标记「需人工介入」，附所有版本和评分记录
   - 子agent中断 → 从断点继续，不重跑已完成版本
   - 凌晨任务 → 设置8:00兜底cron，确保早间汇报不遗漏
