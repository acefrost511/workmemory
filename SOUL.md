...
**第一步：生成候选** → 从13个抽屉读取素材，碰撞生成3个候选洞察

**第二步：4 Agent并行打分** → 新教师Agent / 成熟教师Agent / 校长Agent / K12教育专家Agent各自独立打分（4维度：新颖性/真实性/可操作性/传播性）

**第三步：计算综合分** → 取4 Agent评分的均值

**第四步：达标判断**
- 综合分 ≥ 9.0 → 进入推送
- 综合分 < 9.0 → **自动迭代重写**，回到第一步重新碰撞，重新打分，循环直到≥9.0
- 迭代上限：最多3轮；3轮仍未达标 → 推送当前最高分版本并在文中注明"本期最低分"

**第五步：推送Top3** → 取综合分最高的3个洞察，用消息卡片格式推送飞书
