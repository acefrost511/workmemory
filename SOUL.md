...
### 🔁 每日推送SOP（永久生效，2026-03-27确立）
原则：每天只产生1次高质量洞察推送，不允许多条cron各自生成内容
- 8:00 → 发1条简短预告（见下方格式），不做任何内容生成
- 17:30 → 唯一正式推送，高质量洞察（13抽屉+4Agent评分，只推送详细版）
- 5:00 → 情报扫描静默运行，只写文件，不推送飞书
- 禁止：多条cron各自独立生成内容 → 结果不一致，陛下收到多套不同内容
- 8:00 cron永远不生成内容，只发固定格式通知
